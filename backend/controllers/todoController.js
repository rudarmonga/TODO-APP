...
exports.getTodos = catchAsync(async (req, res, next) => {
  const transaction = PerformanceMonitor.startTransaction('Get Todos', 'db.query');
  ...
  PerformanceMonitor.trackDatabaseQuery('find', 'todos', duration);
  transaction.finish();
...
exports.createTodo = catchAsync(async (req, res, next) => {
  const transaction = PerformanceMonitor.startTransaction('Create Todo', 'db.insert');
  const startTime = process.hrtime();
  ...
  const endTime = process.hrtime(startTime);
  const duration = (endTime[0] * 1e3 + endTime[1] * 1e-6);
  PerformanceMonitor.trackDatabaseQuery('insert', 'todos', duration);
  transaction.finish();
...
exports.updateTodo = catchAsync(async (req, res, next) => {
  const transaction = PerformanceMonitor.startTransaction('Update Todo', 'db.update');
  const startTime = process.hrtime();
  ...
  const endTime = process.hrtime(startTime);
  const duration = (endTime[0] * 1e3 + endTime[1] * 1e-6);
  PerformanceMonitor.trackDatabaseQuery('update', 'todos', duration);
  transaction.finish();
...
exports.deleteTodo = catchAsync(async (req, res, next) => {
  const transaction = PerformanceMonitor.startTransaction('Delete Todo', 'db.delete');
  const startTime = process.hrtime();
  ...
  const endTime = process.hrtime(startTime);
  const duration = (endTime[0] * 1e3 + endTime[1] * 1e-6);
  PerformanceMonitor.trackDatabaseQuery('delete', 'todos', duration);
  transaction.finish();
...
exports.getTodo = catchAsync(async (req, res, next) => {
  const transaction = PerformanceMonitor.startTransaction('Get Single Todo', 'db.query');
  const startTime = process.hrtime();
  ...
  const endTime = process.hrtime(startTime);
  const duration = (endTime[0] * 1e3 + endTime[1] * 1e-6);
  PerformanceMonitor.trackDatabaseQuery('findOne', 'todos', duration);
  transaction.finish();
